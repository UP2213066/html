    ...
    <main>
        ...
        if ($result->num_rows > 0) {
            ...
            while ($row = $result->fetch_assoc()) {
                ...
                $course = htmlspecialchars($row['courseCode']);
                $module = htmlspecialchars($row['moduleCode']);
                $supervisor = htmlspecialchars($row['supervisor']) ?? "NO SUPERVISOR";
                $moderator = htmlspecialchars($row['moderator']) ?? "NO MODERATOR";
                echo "<th>$id</th>";
                echo "<th>$name</th>";
                echo "<th>$course</th>";
                echo "<th>$module</th>";
        ...
        if ($result->num_rows > 0) {
            ...
            while ($row = $result->fetch_assoc()) {
                ...
                $lastName = htmlspecialchars($row['lastName']);
                $name = $firstName . ' ' . $lastName;
                $placementEndYear = htmlspecialchars($row['placementEndYear']);
                echo "<th>$id</th>";
                echo "<th>$name</th>";
